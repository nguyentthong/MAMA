...
from text_video_retrieval.src.modeling.VidCLIP import VidCLIP
import json
from text_video_retrieval.src.datasets.dataset_video_retrieval import (
    HDVILAVideoRetrievalDataset, VideoRetrievalCollator)
from text_video_retrieval.src.datasets.dataloader import InfiniteIterator, PrefetchLoader
from text_video_retrieval.src.datasets.data_utils import ImageNorm, mk_input_group

from text_video_retrieval.src.configs.config import shared_configs
from text_video_retrieval.src.utils.misc import set_random_seed, NoOp, zero_none_grad
from text_video_retrieval.src.utils.logger import LOGGER, TB_LOGGER, add_log_to_file, RunningMeter
from text_video_retrieval.src.utils.basic_utils import (
    load_jsonl, load_json, save_json, get_rounded_percentage)
from text_video_retrieval.src.utils.basic_utils import flat_list_of_lists
from text_video_retrieval.src.utils.load_save import (ModelSaver,
                                 BestModelSaver,
                                 save_training_meta,
                                 load_state_dict_with_mismatch)
from text_video_retrieval.src.utils.load_save import E2E_TrainingRestorer as TrainingRestorer
from text_video_retrieval.src.optimization.sched import get_lr_sched
from text_video_retrieval.src.optimization.utils import setup_e2e_optimizer
from text_video_retrieval.src.optimization.loss import build_loss_func
from text_video_retrieval.src.utils.distributed import all_gather_list
from text_video_retrieval.src.utils.metrics import cal_cossim, compute_metrics, compute_metrics_multi, np_softmax
from tqdm import tqdm
...
def validate(model, val_loaders, cfg):
    ...
    for loader_name, val_loader in val_loaders.items():
        ...
        for val_step, batch in enumerate(tqdm(val_loader)):
        ...
        for type in ["simple", "DSL"]:
            ...
            dataset_name = cfg['val_datasets'][0]['name']
            
            v2t_clean_file = os.path.join(cfg['data_dir'], "video2text_test1ka.jsonl")
            t2v_clean_file = os.path.join(cfg['data_dir'], "text2video_test1ka.jsonl")

            with open(v2t_clean_file, 'r') as f: 
                v2t_eval_lines = f.readlines()
                v2t_eval_data = []
                for line in v2t_eval_lines: v2t_eval_data.append(json.loads(line))

            video_list = [data['clip_id'] for data in v2t_eval_data]

            with open(t2v_clean_file, 'r') as f: 
                t2v_eval_lines = f.readlines()
                t2v_eval_data = []
                for line in t2v_eval_lines: t2v_eval_data.append(json.loads(line))
            text_list = [data['text'] for data in t2v_eval_data]

            v2t_label_dict = {}
            for i in range(len(v2t_eval_data)):
                v2t_label_dict[i] = []
                for text in v2t_eval_data[i]['text']: v2t_label_dict[i].append(text_list.index(text))
                v2t_label_dict[i] = set(v2t_label_dict[i])

            t2v_label_dict = {}
            for i in range(len(t2v_eval_data)):
                t2v_label_dict[i] = []
                for video in t2v_eval_data[i]['clip_id']: t2v_label_dict[i].append(video_list.index(video))
                t2v_label_dict[i] = set(t2v_label_dict[i])
            ...
            top1_v2t_results = np.argsort(-sim_matrix.T, -1)[:,0]
            top1_t2v_results = np.argsort(-sim_matrix, -1)[:,0]
            top5_v2t_results = np.argsort(-sim_matrix.T, -1)[:,:5]
            top5_t2v_results = np.argsort(-sim_matrix, -1)[:,:5]
            top10_v2t_results = np.argsort(-sim_matrix.T, -1)[:,:10]
            top10_t2v_results = np.argsort(-sim_matrix, -1)[:,:10]

            v2tr1 = 0
            for i in range(len(top1_v2t_results)):
                if top1_v2t_results[i] in v2t_label_dict[i]: 
                    v2tr1 += 1
                    continue
            v2tr1 /= len(top1_v2t_results)

            t2vr1 = 0
            for i in range(len(top1_t2v_results)):
                if top1_t2v_results[i] in t2v_label_dict[i]: 
                    t2vr1 += 1
                    continue
            t2vr1 /= len(top1_t2v_results)

            v2tr5 = 0
            for i in range(len(top5_v2t_results)):
                for ans in top5_v2t_results[i]:
                    if ans in v2t_label_dict[i]: 
                        v2tr5 += 1
                        break
            v2tr5 /= len(top5_v2t_results)

            t2vr5 = 0
            for i in range(len(top5_t2v_results)):
                for ans in top5_t2v_results[i]:
                    if ans in t2v_label_dict[i]: 
                        t2vr5 += 1
                        break
            t2vr5 /= len(top5_t2v_results)

            v2tr10 = 0
            for i in range(len(top10_v2t_results)):
                for ans in top10_v2t_results[i]:
                    if ans in v2t_label_dict[i]: 
                        v2tr10 += 1
                        break
            v2tr10 /= len(top10_v2t_results)

            t2vr10 = 0
            for i in range(len(top10_t2v_results)):
                for ans in top10_t2v_results[i]:
                    if ans in t2v_label_dict[i]: 
                        t2vr10 += 1
                        break
            t2vr10 /= len(top10_t2v_results)
...
def start_training():
    ...
    torch.save(cfg, os.path.join('/'.join(cfg['train_datasets']['txt'].split('/')[:-1]), 'cfg.pt'))
    blob_mount(cfg)
    ...
    for step, batch in enumerate(tqdm(InfiniteIterator(train_loader))):
        outputs = model(**batch)
...
# CUDA_VISIBLE_DEVICES=12,13,14,15 horovodrun -np 4 python -m src.tasks.run_video_retrieval --config /home/anhtuan/Thong/ay2324_projects/vidl_projects/xpretrain/clipvip_instances/CLIP-ViP/src/configs/covr_retrieval/covr2m_retrieval_vip_base_32.json --blob_mount_dir /home/anhtuan/Thong/ay2324_projects/vidl_projects/blob_mount
