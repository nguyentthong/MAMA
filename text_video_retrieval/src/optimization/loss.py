...
class NCELearnableTempLoss(nn.Module):
    ...
    def calculate_arccos(self, cos_sim):
        ...
        selected_labels = torch.where(torch.gt(theta, self.margin), torch.ones_like(one_hot_labels), one_hot_labels) * torch.abs(one_hot_labels - 1)
    ...
    def forward(self, vis_feat, text_feat, temp):
        ...
        t2v = self.calculate_arccos(torch.matmul(vis_feat, text_feat.permute(1, 0))) * logit_scale  # temperature
        v2t = t2v.permute(1, 0)
        t2v_label = torch.arange(t2v.shape[0], device=t2v.device)
        v2t_label = t2v_label
        loss = (F.cross_entropy(t2v, t2v_label, reduction='none') + F.cross_entropy(v2t, v2t_label, reduction='none'))
        return loss
