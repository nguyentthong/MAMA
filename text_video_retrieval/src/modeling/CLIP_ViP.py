...
class Adapter(nn.Module):
    def __init__(self, d_model, bottleneck, dropout=0.0, init_option='bert', adapter_scalar=1.0, adapter_layer_norm_option='in'):
        super().__init__()
        self.n_embd = d_model
    ...
    def forward(self, x, add_residual=True, residual=None):
        ...
        if self.adapter_layer_norm_option == 'in':
        ...
        up = up * self.scale
        if self.adapter_layer_norm_before == 'out':
...
class CLIPAttention(nn.Module):
    ...
    def __init__(self, config, adapter_dim=2048):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
        ...
        self.adaptmlp = Adapter(config.hidden_size, adapter_dim)
    ...
    def forward(
        ...
        attn_output = self.adaptmlp(attn_output)
...
class CLIPMLP(nn.Module):
    def __init__(self, config, adapter_dim=2048):
        ...
        self.adaptmlp_1 = Adapter(config.intermediate_size, adapter_dim)
        self.adaptmlp_2 = Adapter(config.hidden_size, adapter_dim)

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        hidden_states = self.fc1(hidden_states)
        hidden_states = self.adaptmlp_1(hidden_states)
        hidden_states = self.activation_fn(hidden_states)
        hidden_states = self.fc2(hidden_states)
        hidden_states = self.adaptmlp_2(hidden_states)
